...
export default function App() {
  ...
  const filtered = notes.filter((item) => {
    if (showCompleted) {
      return completedNotes[notes.indexOf(item)] && item.toLowerCase().includes(search.toLowerCase());
    }
  ...
  return (
    <div className={`${darkMode ? `bg-gray-900  text-white` : `bg-gray-300 text-black`}`}>
      <div className="min-h-screen  flex items-center justify-center p-4">
        <div className={`p-6 rounded-lg shadow-lg w-full max-w-md ${darkMode ? `bg-gray-800  text-white` : `bg-gray-100 text-black`}`}>
          <Button
            variant="text"
            size="small"
            color="primary"
            startIcon={darkMode ? <WbSunnyIcon style={{ color: "white" }} /> : <DarkModeIcon style={{ color: "gray" }} />}
            onClick={() => {
              setDarkMode(!darkMode);
            }}
            className="pl-2 float-right"
          ></Button>
          <h1 style={{ fontFamily: "'Saira Condensed', sans-serif" }} className="text-2xl font-bold mb-4 text-center">
            TaskTame
          </h1>
          ...
          <form onSubmit={toSubmit} className="mb-4">
            <div className="flex items-center mb-2">
              <input
                type="text"
                className={`flex-1 p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${darkMode ? `text-black` : `text-black`}`}
            ...
            <div className="flex items-center bg-white mb-2 border rounded-lg focus-within:ring-2 focus-within:ring-blue-500">
              ...
              <input
                ...
                className={`flex-1 p-2 focus:outline-none ${darkMode ? `text-black` : `text-black`}`}
              />
            </div>
          </form>
          {check && (
          ...
          <Button
            ...
            sx={{ display: notes.length === 0 ? "none" : "inline-flex", mt: 2 }}
          >
            {showCompleted ? "Show All" : "Show Completed"}
          </Button>
          <ul>
            {filtered.map((note, key) => (
              <li className="flex justify-between items-center p-2 border-b last:border-b-0" key={key}>
                <div className="flex">
                ...
                {editing === key ? (
                  ...
                  <span
                    className={`flex-auto min-w-0 max-w-full break-words border border-gray-300 m-1 p-2 ${
                      completedNotes[key] ? "line-through font-bold text-green-500" : ""
                    }`}
                  >
                    {note}
                  </span>
